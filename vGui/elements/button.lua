...
function element:paint(x, y, w, h)
    ...
    if self:getRadius() > 0 and (rTL or rTR or rBR or rBL) then
        ...
        render.drawRoundedBoxEx(self:getRadius(), x + 1, y + 1, w - 2, h - 2, rTL, rTR, rBL, rBR)
    ...
    render.drawSimpleText(x + (self._align == 1 and w / 2 or 0), y + (self._aligny == 1 and h / 2 or 0), self:getText(), self._align, self._aligny)
